...
try {
    foreach ($channels->findAllActive() as $channel) {
        try { $count = $email->synchronize($channel); $processed += $count; $health->record((int) $channel['cod001'], (int) $channel['cod003'], 'Sincronização', 'Sucesso', $count . ' e-mail(s) sincronizado(s).'); fwrite(STDOUT, sprintf("Canal %d: %d e-mail(s) sincronizado(s).\n", $channel['cod003'], $count)); }
        catch (RuntimeException $exception) { $failures++; $health->record((int) $channel['cod001'], (int) $channel['cod003'], 'Sincronização', 'Falha', $exception->getMessage()); fwrite(STDERR, sprintf("Canal %d: %s\n", $channel['cod003'], $exception->getMessage())); }
    }
